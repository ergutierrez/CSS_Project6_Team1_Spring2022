...
for (iter_num in 1:nsim) {  # create for loop to repeat the three steps above
  ...
  # Create temp df that keeps covariate (student_money) and adds random vector from above
   ...
   prop.treat1 <- (temp_df %>% filter(student_money==1 & random_assignment==1) %>% nrow())/(temp_df %>% filter(student_money==1) %>% nrow())
   
   prop.treat0 <- (temp_df %>% filter(student_money==0 & random_assignment==1) %>% nrow())/(temp_df %>% filter(student_money==0) %>% nrow())
...
# KQ said we don't need to include these graphs, just the one above. So we can probably delete
