...
temp_df = ypsps %>% 
  select(student_money) %>% ## 1 = college and 0 = no college
  mutate(rdm_vtr)

# Visualize the distribution by treatment/control (ggplot)
ggplot(temp_df, aes(x = student_money, fill = factor(rdm_vtr))) +
  geom_bar() +
  facet_grid(rdm_vtr~.) +
  labs(title = "Distribution of Student Donations", fill = "Random \nassignment\n")

# Simulate this 10,000 times (monte carlo simulation - see R Refresher for a hint)
distr_vec <- vector()  # create empty vector to append distributions across simulations

for (iter_num in 1:10000) {  # create for loop to repeat the three steps above
   rdm_vtr <- sample(0:1, size = nrow(ypsps), replace = TRUE, prob = c(0.5,0.5))
   ...
   distr_vec <- c(distr_vec, 
                  (sum(temp_df[which(temp_df$rdm_vtr==1), 1]) / nrow(temp_df))
                  ) # get distribution of donations among treated (ie, rdm_vtr==1)
}
...
df <-  ypsps %>% 
  select(college, student_GPA, student_Knowledge, student_NextSch, student_SchClub, student_Newspaper, parent_EducHH, parent_Employ, parent_Knowledge, parent_HHInc, parent_Newspaper)
...
ggplot(df, aes(x = student_GPA, fill = factor(college))) + 
  geom_bar() +
  facet_grid(college~.) +
  labs(title = "Distribution of Student GPA among Treated and Untreated", fill = "A\n")

ggplot(df, aes(x = student_Knowledge, fill = factor(college))) + 
  geom_bar() +
  facet_grid(college~.) +
  labs(title = "Distribution of Student Knowledge among Treated and Untreated", fill = "A\n")

ggplot(df, aes(x = student_NextSch, fill = factor(college))) + 
  geom_bar() +
  facet_grid(college~.) +
  labs(title = "Distribution of Student Next School Year Plans among Treated and Untreated", fill = "A\n")

ggplot(df, aes(x = student_SchClub, fill = factor(college))) + 
  geom_bar() +
  facet_grid(college~.) +
  labs(title = "Distribution of Student Attendance to School Clubs among Treated and Untreated", fill = "A\n")

ggplot(df, aes(x = student_Newspaper, fill = factor(college))) + 
  geom_bar() +
  facet_grid(college~.) + 
  labs(title = "Distribution of student_Newspaper among Treated and Untreated", fill = "A\n")

ggplot(df, aes(x = parent_EducHH, fill = factor(college))) + 
  geom_bar() +
  facet_grid(college~.) + 
  labs(title = "Distribution of Parents' Household Education Level among Treated and Untreated", fill = "A\n")

ggplot(df, aes(x = parent_Employ, fill = factor(college))) + 
  geom_bar() +
  facet_grid(college~.) +
  labs(title = "Distribution of Parental Employment among Treated and Untreated", fill = "A\n")

ggplot(df, aes(x = parent_Knowledge, fill = factor(college))) + 
  geom_bar() +
  facet_grid(college~.) + 
  labs(title = "Distribution of Parental Knowledge among Treated and Untreated", fill = "A\n")

ggplot(df, aes(x = parent_HHInc, fill = factor(college))) + 
  geom_bar() +
  facet_grid(college~.) +
  labs(title = "Distribution of Parents' Household Income among Treated and Untreated", fill = "A\n")

ggplot(df, aes(x = parent_Newspaper, fill = factor(college))) + 
  geom_bar() +
  facet_grid(college~.) +
  labs(title = "Distribution of parent_Newspaper among Treated and Untreated", fill = "A\n")

# Report the overall balance
ggplot(df, aes(x = prop_score, fill = factor(college))) + 
  geom_histogram() 
  labs(title = "Distribution of Propensity Score among Treated and Untreated")

# Report the proportion of covariates that meet the balance threshold
model_matchit <- matchit(college ~ student_GPA + student_Knowledge + student_NextSch + student_SchClub + student_Newspaper + parent_EducHH + parent_Employ + parent_Knowledge + parent_HHInc + parent_Newspaper,
                        data = df,
                        replace = TRUE)
summary(model_matchit)
...
df2 <- ypsps %>% select(!contains("1973") & !contains("1982") & !"interviewid" & !"college")

# Randomly select features
dfSIM <- sample(df2, size = sample(1:ncol(df2)), replace=FALSE) # Solis: I changed this to replace=FALSE, I don't think it works if the same covariates are repeated
college <- ypsps %>% select(college)

df3 <- cbind(college, dfSIM) 
df3.complete <- df3 %>% filter(complete.cases(.)) # keep only nonmissing

#I am not sure how to regress the dependent variable on "randomized" features here, so I'm stuck for now..    
# Solis: I think you can use the "." to regress on all other covariates except the independent variable. Which should be whatever the randomized features are
model_matchit <- matchit(college ~ .,
                        data = df3, 
                        estimand = "ATT",
                        replace = TRUE)
```
...
```{r}
# Solis: I made some edits to the simulation, I think it should be something like this??
...
# Keep complete cases only; matchit won't run with missing covariate data 
ypsps <- ypsps %>% filter(complete.cases(.))

# Create empty matrix to populate ATTs, proportion of balanced covariates, and mean percent balance improvement
model.output <- matrix( , nrow = 10000, ncol = 3)
names(model.output) <- c("ATT", "Prop Balanced Covariates", "Mean Percent Balance Improvement")

for (sim_num in 1:10000) {
  ...
  # Save ATTs, proportion of balanced covariates, and mean percent balance improvement
    ...
    model.output[num_sim, 1] <- ps_att_summ$coefficients["college", "Estimate"]
    model.output[num_sim, 2] <- # need to add code to get the proportion of balanced covariates
    model.output[num_sim, 3] <- # need to add code to get the mean percent balance improvement
...
# Randomly select features
dfSIM <- sample(df2, size = sample(1:ncol(df2)), replace=TRUE)
df3 <- cbind(college, dfSIM)

# Fit  models and save ATTs, proportion of balanced covariates, and mean percent balance improvement
model_matchit <- matchit(college ~ colnames(dfSIM),
                        data = df3, 
                        estimand = "ATT",
                        replace = TRUE)
}
