...
for (sim_num in 1:10000) {
    ...
    dfSIM <- sample(df2, size = sample(1:ncol(df2)), replace=FALSE)
    df3 <- cbind(college, dfSIM)
    df3.complete <- df3 %>% filter(complete.cases(.)) # keep only nonmissing

    psm_matchit <- matchit(college ~ .,
                        data = df3.complete, 
                        estimand = "ATT",
                        replace = TRUE)
    
    psm_ate_data <- match.data(psm_matchit)
    lm_exact_ate <- lm(college ~ ., data = psm_ate_data, weights = weights)
