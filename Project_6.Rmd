...
\begin{enumerate}
    ...
    \item \textbf{Your Answer:} Looking at the scatterplot of the proportion of balanced covariates and ATTs, it seems like more than half of the models have a balanced covariates proportion higher than 0.5. Because the distribution of balance is wide across samples and models of the simulation, this creates concerns around sample-specific estimates. Even when we choose a model with high balance, problems of generalizability arise due to the estimates being sample and model specific.
    \item \textbf{Analyze the distribution of the ATTs. Do you have any concerns about this distribution?}
    \item \textbf{Your Answer:} The ATTs from simulations have a bimodal distribution. Combining the scatterplot and the ATT histogram, one can argue that the proportion of balanced covariates is correlated with the magnitude of ATT: lower proportion of balanced covariates is associated with near-zero ATT, while higher proportion of balanced covariates is associated with higher ATT. This becomes a concern because our propensity score models result in sample-specific estimates of ATT. 
...
\begin{enumerate}
    ...
    \item \textbf{Your Answer:} Looking at the scatterplot of the proportion of balanced covariates and ATTs of the alternative matching method, it seems like the majority of models have less than 3 balanced covariates. This suggests balance overall is worse in the alternative models compared to propensity score models in this case.
...
\begin{enumerate}
    ...
    \item \textbf{Your Answer:} Matching is a solution to the problem of sparseness. In real world, although we have a randomized design, it is impossible to make sure that the units are identical in all possible relevant characteristics except treatment status. That’s why including probability of treatment based on measured characteristics in estimation makes sense.
    ...
    \item \textbf{Your Answer:} As machine learning algorithms are more complex combinations of simpler models such as logistic regression, there might be benefits to estimating propensity scores using the mentioned models as long as there are enough units of observation for accurate machine learning predictions.
\end{enumerate}